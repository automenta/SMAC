...
public class SMACTester {
	...
	public static String smacDeployment = "/tmp";
	...
	public static void initDeploymentToTest()
	{
		...
		public String getExecString(String scenarioFile, boolean adaptiveCapping, int iterationLimit, boolean ROARMode, int restoreIteration, int id, boolean verifyHashCodes, boolean checkInstances, boolean surrogate)
		{
			...
			*/
			...
			String execString = "--cleanOldStateOnSuccess false --scenarioFile " +  scenarioFile + " --numIterations " + iterationLimit + " --runGroupName "  + runID + "-" + id + " --experimentDir " + experimentDir + " --numRun 0 "  +  instanceCheck+  " --doValidation false --seedOffset " + seedOffset;
		...
		public int runSMAC(String scenarioFile, boolean adaptiveCapping, int iterationLimit, boolean ROARMode, int restoreIteration, int id, String messageClass, String message, boolean verifyHashCodes, boolean checkInstances, boolean surrogate )
		{
			...
			try {
				...
				//--cleanOldStateOnSuccess false --scenarioFile /tmp/example_spear/scenario-Spear-SWGCP-sat-small-train-small-test.txt --numIterations 18 --runGroupName scenario-Spear-SWGCP-sat-small-train-small-test.txt-JUNIT-0 --experimentDir /tmp --numRun 0  --checkInstanceFilesExist false  --doValidation false --seedOffset 8777282093646094225 --executionMode ROAR

				SMACOptions options = new SMACOptions();
				//options.scenarioConfig.scenarioFile = get
				SMACExecutor.run(options);

					BufferedReader in = new BufferedReader(new InputStreamReader(new PipedInputStream(po))); //new InputStreamReader(p.getInputStream()));
